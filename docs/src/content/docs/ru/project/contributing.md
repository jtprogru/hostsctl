...
- `gen-check` пересобирает сгенерированные страницы справочника из бинаря и падает, если
  закоммиченная копия отличается. Тронул определение CLI или коды выхода — запусти
  `make gen` и закоммить результат.
...
```
docs/src/parts/<locale>/reference-cli.head.md        текст и frontmatter
+ hostsctl docs cli                                  дерево команд
+ docs/src/parts/<locale>/reference-cli.tail.md      текст
= docs/src/content/docs/[ru/]reference/cli.md        этот файл не править
```

`reference/exit-codes.md` собирается так же. Правь части и запускай `make gen`; собранный
